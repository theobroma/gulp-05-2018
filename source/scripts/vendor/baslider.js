    ...
    beforeAfter: function() {
      $(this).each(function() {
        ...
        $('img:first', baContainer)
          .wrap('<div class="ba-slider-before-image"/>')
          .width(afterImageW);
        $('.ba-slider-before-image', baContainer).css({ width: percNum(percentage) });
        $('img', baContainer).addClass('ba-slider-image');

        html =
          '<div class="ba-control"><div class="ba-control-area"><div class="ba-control-bar"><span class="ba-left-arrow" /><span class="ba-control-handle" /><span class="ba-right-arrow" /></div></div></div>';
        ...
        baBar.on('mousedown', function(e) {
          ...
          $(document)
            ...
            .on('mouseup', stopHandler);
        });

        // Hover
        baContainer.on('mouseover', function(e) {
          $('.ba-left-arrow, .ba-right-arrow, .ba-control-handle', baContainer)
            .filter(':not(:animated)')
            .fadeIn();
        });
        ...
        baContainer.on('click', function(e) {
          ...
          $('.ba-slider-before-image', baContainer)
            .stop()
            .animate({
              width: percNum(posX)
            });
